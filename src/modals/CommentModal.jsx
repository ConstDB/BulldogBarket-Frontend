...
const CommentModal = forwardRef(({ listing }, ref) => {
  ...
  const isSeller = (commenterId) => listing.seller.id === commenterId;
  ...
  return (
    <dialog
      ref={dialogRef}
      className="w-[420px] max-h-[90vh] fixed inset-0 m-auto rounded-xl overflow-hidden p-4 bg-white shadow-lg"
      onClick={(e) => {
      ...
      onCancel={(e) => {
        e.preventDefault(); // keep control consistent
        dialogRef.current.close();
      }}
    >
      {/* Header */}
      ...
      {/* Comments */}
      <div className="min-h-[430px] max-h-[450px] w-[390px] flex-1 overflow-y-auto pt-2 pr-1 space-y-6">
          ...
          comments.map((c) => (
            <div key={c._id} className="flex items-start gap-2.5">
              ...
              <div className="flex flex-col w-full">
                ...
                <div className="max-w-[320px] text-[14px] leading-[18px] wrap-anywhere">
                  <p>{c.message}</p>
                </div>
